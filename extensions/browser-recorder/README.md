...
From the **repository root** (`chrome-extensions/`):
...
3. In Chrome:
   ...
   - Click **Load unpacked**
   - Select **`extensions/browser-recorder/dist/`** (this folder’s build output)
   - If already loaded, click **Reload**

4. Optional quality checks (still from repository root):
...
- Advanced settings include semantic label capture level (`Off`, `Minimal`, `Medium`, `Full`; default `Minimal`) to reduce generic UI noise.
...
- This folder includes `recordings/.gitkeep` so the recordings path exists in the tree.
...
- URL query params containing sensitive key names are redacted in exported page metadata.
...
- Export writes to Chrome’s download target (not directly to this repository path) due to extension sandboxing.
